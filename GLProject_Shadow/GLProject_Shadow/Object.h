...
public:
	...
	void PaintShadow(float vertex[][3], float lightPos[3]);

	//计算向光面
	void SetLightedFaces(float lightPos[3]); //必须先调用setPlaneEq()

protected:
	int shadowDepth;

	//Z-Pass算法
	void DoShadowPass(float vertex[][3], float lightPos[3]);
